...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE key_value_map(key TEXT PRIMARY KEY, value TEXT)");
  }
  ...
  Future<KeyValueMap> selectKeyValueMap(String key) async {
    ...
    return result.isNotEmpty
        ? KeyValueMap.fromMap(result.first)
        : KeyValueMap(); // 1개만 나오는게 정상
  }
  ...
  Future<int> updateKeyValueMap(String paramKey, String paramValue) async {
    Database db = await instance.database;
    return await db.update('key_value_map', {'value': paramValue},
        where: 'key = \'$paramKey\'');
  }

  Future<int> deleteKeyValueMap(String paramKey) async {
    Database db = await instance.database;
    return await db.delete('key_value_map', where: 'key = ?', whereArgs: [paramKey]);
  }
}
