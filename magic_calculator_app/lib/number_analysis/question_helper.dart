...
class QuestionMaker {
  ...
  List<QuestionCase> getBestQuestionSet(List<QuestionCase> resultList){
    ...
    List<QuestionCase> bestResult = List<QuestionCase>.from(resultList.where((questionCase) => questionCase.worstCount == 1));
  ...
  getFirstDepthQuestionCase(List<QuestionCase> resultList) {
    for (int i = 0; i < matrixSize; i++) {
      ...
      QuestionCandidate questionCandidate =
          QuestionCandidate.values[i + modifier];
      candidateList.add(questionCandidate);
      int maxValue = numberStatistics.makeOptimalQuestion(candidateList);
      //resultMap.put(candidateList, maxValue);
      resultList.add(new QuestionCase(candidateList, maxValue));
    }
  }
  ...
  getSecondDepthQuestionCase(List<QuestionCase> resultList) {
    for (int i = 0; i < matrixSize; i++) {
      ...
      for (int j = 0; j < matrixSize; j++) {
        QuestionCandidate questionCandidate2 =
            QuestionCandidate.values[j + modifier];
        if (candidateList.contains(questionCandidate2)) {
          continue;
        }
        List<QuestionCandidate> candidateList2 =
            List<QuestionCandidate>.from(candidateList);
        candidateList2.add(questionCandidate2);
        int maxValue = numberStatistics.makeOptimalQuestion(candidateList2);
    ...
    for (int i = 0; i < matrixSize; i++) {
      ...
      for (int j = 0; j < matrixSize; j++) {
        ...
        for (int k = 0; k < matrixSize; k++) {
          QuestionCandidate questionCandidate3 =
              QuestionCandidate.values[k + modifier];
          if (candidateList2.contains(questionCandidate3)) {
            continue;
          }
          List<QuestionCandidate> candidateList3 =
              List<QuestionCandidate>.from(candidateList2);
          candidateList3.add(questionCandidate3);

          int maxValue = numberStatistics.makeOptimalQuestion(candidateList3);
      ...
      for (int j = 0; j < matrixSize; j++) {
        ...
        for (int k = 0; k < matrixSize; k++) {
          ...
          for (int l = 0; l < matrixSize; l++) {
            QuestionCandidate questionCandidate4 =
                QuestionCandidate.values[l + modifier];
            if (candidateList3.contains(questionCandidate4)) {
              continue;
            }
            List<QuestionCandidate> candidateList4 =
                List<QuestionCandidate>.from(candidateList3);
            candidateList4.add(questionCandidate4);

            int maxValue = numberStatistics.makeOptimalQuestion(candidateList4);
